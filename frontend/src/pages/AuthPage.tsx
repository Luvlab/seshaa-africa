...
export default function AuthPage() {
  ...
  const inputCls = 'w-full border-2 border-white/30 rounded-2xl pl-12 pr-4 py-4 text-base outline-none focus:border-white/70 bg-white/90 text-gray-900 placeholder-gray-400';

  return (
    <div className="min-h-screen flex flex-col">
      {/* Flag gradient — sits above main's bg-gray-50 (z:1) but below navbar (z:50) */}
      <div className="fixed inset-0 z-[1]" style={{ background: 'var(--app-bg, linear-gradient(135deg, #008751 0%, #FCD116 50%, #D90000 100%))' }} />
      {/* Form — above the gradient (z:2) */}
      <div className="relative z-[2] flex-1 flex flex-col justify-center px-5 py-8 max-w-md mx-auto w-full">
        <div className="bg-white/15 backdrop-blur-xl border border-white/20 rounded-3xl p-7 shadow-2xl">
          ...
          <div className="flex bg-white/10 rounded-2xl p-1 mb-6">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-base font-semibold transition-all ${
                inputType === 'phone' ? 'bg-white shadow text-gray-900' : 'text-white/70 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-base font-semibold transition-all ${
                inputType === 'email' ? 'bg-white shadow text-gray-900' : 'text-white/70 hover:text-white'
              }`}
            >
          ...
          <div className="space-y-4">
            {mode === 'register' && (
              <div className="relative">
                ...
                <input type="text" placeholder={t('auth.name')} className={inputCls}
            ...
            <div className="relative">
              ...
              />
              <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 p-1"
            ...
            {mode === 'register' && (
              <div className="relative">
                ...
                <input type="text" placeholder={t('auth.country')} className={inputCls}
            ...
            {mode === 'register' && (
              <>
                <p className="text-sm font-semibold text-white/50 pt-1">Social handles (optional)</p>
                {(['instagram','snapchat','tiktok','facebook'] as const).map((key, i) => (
                  <div key={key} className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-bold">
                      {['IG','SC','TK','FB'][i]}
                    </span>
                    <input type="text"
                      placeholder={['Instagram @username','Snapchat @username','TikTok @username','Facebook handle or profile URL'][i]}
                      className={inputCls}
                      value={form[key]}
          ...
          {/* Error */}
          {error && (
            <div className="mt-4 p-4 bg-red-500/20 border border-red-400/40 rounded-2xl text-white text-sm font-medium">
              {error}
            </div>
          )}
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading
              ...
              : <>{mode === 'login' ? t('auth.loginBtn') : t('auth.registerBtn')}<ArrowRight size={22} /></>
            }
          </button>

          {/* Google OAuth */}
          <button
            className="mt-3 w-full flex items-center justify-center gap-3 py-4 rounded-2xl border-2 border-white/30 bg-white/90 text-gray-700 text-base font-semibold hover:bg-white active:bg-white/80 transition-colors"
            onClick={() => {
          ...
          <div className="mt-8 text-center">
            ...
            <button className="mt-2 text-base font-bold text-white underline underline-offset-2"
